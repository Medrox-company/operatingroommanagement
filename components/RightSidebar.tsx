...
export default function RightSidebar({ rooms, currentTime, isExpanded }: RightSidebarProps) {
  // Výpočty statistik - pokud má sál v daném čase aktivní operaci (currentProcedure)
  const activeRoomsCount = rooms.filter(r => r.currentProcedure && !r.isLocked).length;
  const readyRoomsCount = rooms.filter(r => (r.currentStepIndex === 0 || r.currentStepIndex === 7) && !r.isLocked).length;
  const lockedRoomsCount = rooms.filter(r => r.isLocked).length;
  const emergencyRoomsArray = rooms.filter(r => r.isEmergency);
  const emergencyRoomsCount = emergencyRoomsArray.length;
  
  // Hledáme sály s překročeným časem
  const overtimeRooms = rooms.filter(r => {
    if (!r.currentProcedure || !r.operationStartedAt) return false;
    // Pokud máme odhadovaný konec a je v minulosti
    if (r.estimatedEndTime) {
      const endTime = new Date(r.estimatedEndTime).getTime();
      return currentTime.getTime() > endTime;
    }
    return false;
  });

  const avgOccupancy = rooms.length > 0 ? Math.round((activeRoomsCount / rooms.length) * 100) : 0;
  ...
  return (
    ...
    >
      ...
      <div>
        ...
        <div className="grid grid-cols-2 gap-2">
          ...
          >
            ...
            <div className="text-2xl font-bold text-cyan-300">{activeRoomsCount}</div>
          ...
          >
            ...
            <div className="text-2xl font-bold text-green-300">{readyRoomsCount}</div>
          ...
          >
            ...
            <div className="text-2xl font-bold text-red-300">{lockedRoomsCount}</div>
      ...
      {/* Alarmy */}
      {(emergencyRoomsCount > 0 || overtimeRooms.length > 0) && (
        <motion.div
