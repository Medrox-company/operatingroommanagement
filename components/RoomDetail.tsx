...
const RoomDetail: React.FC<RoomDetailProps> = ({ room, allRooms = [], onClose, onStepChange, onEndTimeChange, onEnhancedHygieneToggle, onStaffChange, onPatientStatusChange }) => {
  ...
  return (
    ...
    >
      {/* ========== MOBILE LAYOUT - Ultra Minimalist Dark Green Design ========== */}
      <div className="md:hidden w-full h-full flex flex-col overflow-hidden relative">
        
        {/* Background - Deep green gradient inspired by reference */}
        <div className="absolute inset-0 bg-gradient-to-b from-[#0d2818] via-[#0a1f14] to-[#050d08]" />
        
        {/* Subtle glow effects */}
        <div className="absolute top-0 left-1/2 -translate-x-1/2 w-[300px] h-[300px] rounded-full bg-emerald-500/10 blur-[100px]" />
        <div className="absolute bottom-0 right-0 w-[200px] h-[200px] rounded-full bg-teal-500/5 blur-[80px]" />
        ...
        <div className="relative z-10 flex flex-col h-full">
          ...
          <div className="px-5 pt-6 pb-4 flex items-center justify-between">
            <button 
              onClick={onClose}
              className="w-10 h-10 rounded-2xl bg-white/5 backdrop-blur-sm flex items-center justify-center border border-white/10"
            >
          ...
          <div className="flex-1 overflow-y-auto px-5 pb-32">
            ...
            {/* Status Card - Hero */}
            <div className="bg-white/[0.03] backdrop-blur-xl border border-white/10 rounded-3xl p-6 mb-5">
              <p className="text-[10px] font-medium text-emerald-400/80 uppercase tracking-[3px] mb-2">
                Aktuální fáze
              </p>
              <h2 className="text-2xl font-bold text-white mb-4">
                {room?.isEmergency ? 'Stav nouze' : room?.isLocked ? 'Uzamčeno' : (currentStep?.name || 'Status')}
              </h2>
              ...
              <div className="h-2 bg-white/10 rounded-full overflow-hidden mb-3">
                <div 
                  ...
                  style={{ 
                    width: activeDbStatuses?.length 
                      ? `${((currentStepIndex + 1) / activeDbStatuses.length) * 100}%`
                      : '0%'
                  }}
                />
              </div>
              <p className="text-xs text-white/40">
                {activeDbStatuses?.length ? `Krok ${currentStepIndex + 1} z ${activeDbStatuses.length}` : 'Načítání...'}
              </p>
            </div>
            ...
            <div className="grid grid-cols-2 gap-3 mb-5">
              ...
              <div className="bg-white/[0.03] backdrop-blur-sm border border-white/10 rounded-2xl p-4">
                <p className="text-[9px] font-medium text-white/40 uppercase tracking-widest mb-2">
                  Uplynulý čas
                </p>
              ...
              <div className="bg-emerald-500/10 border border-emerald-500/20 rounded-2xl p-4">
                <p className="text-[9px] font-medium text-emerald-400/80 uppercase tracking-widest mb-2">
                  Ukončení
                </p>
            ...
            {/* Time Adjustment */}
            <div className="bg-white/[0.03] backdrop-blur-sm border border-white/10 rounded-2xl p-4 mb-5">
              <p className="text-[9px] font-medium text-white/40 uppercase tracking-widest mb-3 text-center">
                Úprava času
              </p>
              <div className="flex items-center justify-between">
                <button
                  onClick={handleDecreaseTime}
                  disabled={isInteractionBlocked || !localEndTime}
                  className="w-12 h-12 rounded-xl bg-white/5 border border-white/10 flex items-center justify-center disabled:opacity-30 active:scale-95"
                >
            ...
            {/* Team */}
            <div className="mb-5">
              <p className="text-[9px] font-medium text-white/40 uppercase tracking-widest mb-3 px-1">
                Tým
              </p>
              <button
                onClick={() => {
                ...
                className="w-full bg-white/[0.03] border border-white/10 rounded-2xl p-4 text-left mb-2 active:bg-white/[0.06]"
              >
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-[10px] text-white/40 uppercase">Lékař</p>
                    <p className="text-white font-medium">{room?.staff?.doctor?.name || 'Neurčeno'}</p>
                  </div>
                </div>
              </button>
              
              {room?.staff?.nurse?.name && (
                <div className="w-full bg-white/[0.03] border border-white/10 rounded-2xl p-4">
                  <div className="flex items-center gap-3">
                    <div className="w-10 h-10 rounded-xl bg-teal-500/20 flex items-center justify-center">
                      <Stethoscope className="w-5 h-5 text-teal-400" />
                    </div>
                    <div>
                      <p className="text-[10px] text-white/40 uppercase">Sestra</p>
                      <p className="text-white font-medium">{room.staff.nurse.name}</p>
                    </div>
                  </div>
                </div>
              )}
            ...
            <div className="grid grid-cols-4 gap-2 mb-5">
              ...
              <button
                onClick={async () => {
                  ...
                  } catch (e) { console.error('[v0] Pause error:', e); }
                }}
                className={`aspect-square rounded-2xl flex flex-col items-center justify-center gap-1 border active:scale-95 ${
                  isPaused 
              ...
              >
                ...
                <span className="text-[8px] text-white/40 uppercase">{isPaused ? 'Spustit' : 'Pauza'}</span>
              </button>
              ...
              <button
                onClick={async () => {
                  ...
                  } catch (e) { console.error('[v0] Hygiene error:', e); }
                }}
                className={`aspect-square rounded-2xl flex flex-col items-center justify-center gap-1 border active:scale-95 ${
              ...
              >
                ...
                <span className="text-[8px] text-white/40 uppercase">Hygiena</span>
              </button>
                ...
                onClick={async () => {
                  try {
                    if (!patientCalledTime) {
                      ...
                      if (onPatientStatusChange) onPatientStatusChange(now.toISOString(), patientArrivedTime?.toISOString() || null);
                    }
                  } catch (e) { console.error('[v0] Patient call error:', e); }
                }}
                disabled={!!patientCalledTime}
                className={`aspect-square rounded-2xl flex flex-col items-center justify-center gap-1 border active:scale-95 disabled:opacity-50 ${
                  patientCalledTime 
                    ? 'bg-blue-500/20 border-blue-500/50' 
                    : 'bg-white/[0.03] border-white/10'
                }`}
              >
                <Phone className={`w-5 h-5 ${patientCalledTime ? 'text-blue-400' : 'text-white/50'}`} />
                <span className="text-[8px] text-white/40 uppercase">Volání</span>
              </button>
                ...
                onClick={async () => {
                  try {
                    if (patientCalledTime && !patientArrivedTime) {
                      ...
                      if (onPatientStatusChange) onPatientStatusChange(patientCalledTime.toISOString(), now.toISOString());
                    }
                  } catch (e) { console.error('[v0] Patient arrived error:', e); }
                }}
              ...
              >
                <UserCheck className={`w-5 h-5 ${patientArrivedTime ? 'text-green-400' : 'text-white/50'}`} />
                <span className="text-[8px] text-white/40 uppercase">Příchod</span>
              </button>
            </div>
          </div>
          ...
          {!isInteractionBlocked && (
            <div className="absolute bottom-0 left-0 right-0 p-5 bg-gradient-to-t from-[#050d08] via-[#050d08]/95 to-transparent">
              <button
                onClick={handleNextStep}
                className="w-full py-4 rounded-2xl bg-gradient-to-r from-emerald-500 to-teal-500 text-white font-bold text-base active:scale-[0.98] shadow-lg shadow-emerald-500/30"
              >
