...
function TimelineModuleImpl({ rooms }: TimelineModuleProps) {
  ...
  // Auto-scroll to current time on mount and when time changes
  useEffect(() => {
    if (scrollContainerRef.current && scrollTarget !== null) {
  ...
  useEffect(() => {
    const target = (currentTimePercent / 100) * (window.innerWidth - SIDEBAR_WIDTH) - 100;
    setScrollTarget(Math.max(0, target));
  }, [currentTime]);
    ...
    <div className="w-full h-full text-white overflow-hidden flex flex-col relative" style={{ background: 'linear-gradient(135deg, rgba(0,10,20,0.98) 0%, rgba(0,20,40,0.92) 100%)' }}>
      ...
      <div className="hidden md:flex md:flex-1 md:flex-col md:overflow-hidden">
        ...
        <div className="flex-shrink-0 flex items-stretch border-b" style={{ borderBottomColor: C.border }}>
          ...
          {/* Sidebar placeholder */}
          <div style={{ width: SIDEBAR_WIDTH }} className="border-r flex-shrink-0 flex items-center px-4" style={{ borderRightColor: C.border }}>
            <div>
              <p className="text-xs" style={{ color: C.muted }}>TIMELINE OPERAČNÍCH SÁLŮ</p>
              <p className="text-lg font-bold mt-1" style={{ color: C.textHi }}>
          ...
          {/* Time markers */}
          <div className="flex-1 flex relative h-20 px-4 items-end overflow-hidden bg-gradient-to-b from-transparent to-blue-950/10">
            {Array.from({ length: 25 }).map((_, i) => (
              <div 
                key={i}
                className="flex-1 border-r pb-3 text-center text-xs relative group"
                style={{ borderRightColor: C.border }}
              >
                <motion.span 
                  className="font-semibold transition-colors"
                  style={{ color: Math.abs(i - currentTimePercent / 4.17) < 0.5 ? '#00F5A0' : C.muted }}
                >
                  {hourLabel(i)}
                </motion.span>
              </div>
            ...
            <motion.div 
              className="absolute top-0 bottom-0 w-1 bg-gradient-to-b from-red-500 via-red-400 to-transparent"
