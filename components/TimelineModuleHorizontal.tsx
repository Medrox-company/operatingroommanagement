                  ...
                  return (
                    <div 
                      ...
                      style={{
                        ...
                        border: `1px solid ${roomColor.bg}40`,
                      }}
                    >
                      <div className="flex items-center gap-2 mb-1">
                        <div 
                          className="w-2 h-2 rounded-full flex-shrink-0"
                          style={{ backgroundColor: status ? C.green : C.muted }}
                        />
              ...
              {sortedRooms.map((room, idx) => {
                ...
                return (
                  <div key={room.id} className="relative h-12 flex items-center">
                    ...
                    {status && (
                      <motion.div
                        className="absolute top-1/2 -translate-y-1/2 rounded-lg border transition-all duration-200 group cursor-pointer"
                        style={{
                          backgroundColor: `${roomColor.bg}80`,
                          borderColor: roomColor.border,
                          height: '32px',
                          left: `${getOperationPosition(
                            new Date(status.start_datetime || Date.now()),
                            new Date(status.end_datetime || Date.now()),
                            currentTime
                          ).left}%`,
                          width: `${getOperationPosition(
                            new Date(status.start_datetime || Date.now()),
                            new Date(status.end_datetime || Date.now()),
                            currentTime
                          ).width}%`,
                      ...
                      >
                        ...
                        {status.status === 'active' && (
                          <motion.div
                            className="absolute -top-1 -right-1 w-4 h-4 rounded-full bg-green-500 border border-green-300"
                            animate={{ scale: [1, 1.2, 1] }}
                            transition={{ duration: 2, repeat: Infinity }}
                          >
                            <span className="text-xs text-white font-bold absolute inset-0 flex items-center justify-center">●</span>
                          </motion.div>
                        )}
                      </motion.div>
                    )}
