  ...
  return (
    ...
    >
      <motion.div
        ...
        className="w-full h-full md:h-auto md:max-w-md flex flex-col md:rounded-3xl overflow-hidden"
        style={{
          background: `linear-gradient(180deg, rgba(10,15,30,0.95) 0%, rgba(15,25,40,0.95) 100%)`,
        }}
      >
        {/* Header - minimal */}
        ...
        <div className="flex-1 flex flex-col items-center justify-center px-6 py-8 space-y-8">
          ...
          >
            ...
            <svg className="absolute w-full h-full -rotate-90" viewBox="0 0 120 120">
              ...
              <circle 
                cx="60" cy="60" r="50" fill="none" stroke={stepColor} strokeWidth="8"
            ...
            <div className="absolute inset-0 flex flex-col items-center justify-center">
              <div className="text-5xl font-black text-white" style={{ color: stepColor }}>
                {getElapsedTime()}
              </div>
              <p className="text-xs text-white/50 uppercase tracking-widest mt-2">DOBA POSTUPU</p>
            </div>
          </motion.div>
          ...
          <div className="text-center space-y-3">
            ...
            >
              <p className="text-3xl font-black text-white">{currentStatus?.title || 'Status'}</p>
          ...
          <div className="flex items-center gap-2">
            {activeStatuses.map((_, idx) => (
              <motion.div
                ...
                animate={{
                  width: idx <= stepIndex ? 24 : 6,
                  backgroundColor: idx <= stepIndex ? stepColor : 'rgba(255,255,255,0.15)',
                }}
              />
            ))}
          </div>
        ...
        {nextStatus && (
          <div className="px-6 py-4 border-t border-white/5 bg-white/[0.02]">
            <p className="text-xs text-white/40 uppercase tracking-wider mb-2">DALŠÍ KROK</p>
            <p className="text-sm font-semibold text-white/70">{nextStatus?.title || 'Waiting'}</p>
          </div>
        )}

        {/* Close hint - mobile only */}
        <div className="md:hidden px-6 py-4 text-center">
          <p className="text-xs text-white/30">Klepněte pro zavření</p>
        </div>
      </motion.div>
    </motion.div>
  );
