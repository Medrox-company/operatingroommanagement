...
import { BarChart3, Calendar, TrendingUp, Users, Clock, Building2, Activity, ChevronDown, Zap, Target } from 'lucide-react';
import { OperatingRoom } from '../types';
import { WORKFLOW_STEPS, MOCK_ROOMS } from '../constants';
import { LineChart, Line, BarChart, Bar, ResponsiveContainer, XAxis, YAxis, CartesianGrid, Tooltip, AreaChart, Area, PieChart, Pie, Cell, RadarChart, PolarGrid, PolarAngleAxis, PolarRadiusAxis, Radar } from 'recharts';
...
const StatisticsModule: React.FC<StatisticsModuleProps> = ({ rooms = MOCK_ROOMS }) => {
  ...
  const phaseData = [
    { phase: 'Přijezd→Anestezie', duration: 12, color: WORKFLOW_STEPS[0]?.color || '#818CF8', label: 'Příprava' },
    { phase: 'Anestezie→Výkon', duration: 8, color: WORKFLOW_STEPS[1]?.color || '#A855F7', label: 'Anestezie' },
    { phase: 'Chirurgický výkon', duration: 95, color: WORKFLOW_STEPS[2]?.color || '#06B6D4', label: 'Výkon' },
    { phase: 'Výkon→Konec anestezie', duration: 15, color: WORKFLOW_STEPS[3]?.color || '#10B981', label: 'Probuzení' },
    { phase: 'Anestezie→Úklid', duration: 10, color: WORKFLOW_STEPS[4]?.color || '#F97316', label: 'Podchyt' },
    { phase: 'Úklid→Připraveno', duration: 20, color: WORKFLOW_STEPS[5]?.color || '#FBBF24', label: 'Úklid' },
  ];

  const totalDuration = phaseData.reduce((sum, p) => sum + p.duration, 0);

  // Radar chart data - efficiency metrics
  const radarData = [
    { metric: 'Časová efektivita', value: 89 },
    { metric: 'Vytížení', value: 87 },
    { metric: 'Kvalita procesu', value: 92 },
  ...
  // Get color for heatmap based on utilization
      ...
      <div className="flex-1 overflow-y-auto hide-scrollbar">
        {selectedRoom && (
          <div className="space-y-8 py-8 px-2">
            ...
            {/* Phase Duration Breakdown - Waterfall style */}
            <motion.div
              className="p-6 rounded-2xl border backdrop-blur-sm"
              style={{ borderColor: 'rgba(255, 255, 255, 0.1)', background: 'rgba(255, 255, 255, 0.04)' }}
              initial={{ opacity: 0, y: 20 }}
            ...
            >
              <h2 className="text-lg font-black text-white mb-6 flex items-center gap-2">
                ...
                Rozpis jednotlivých fází operačního cyklu - Časová osa
              </h2>
...
const StatisticsModule: React.FC<StatisticsModuleProps> = ({ rooms = MOCK_ROOMS }) => {
  ...
  // Phase data for selected room
  const phaseData = [
    { phase: 'Přijezd→Anestezie', duration: 12, color: WORKFLOW_STEPS[0]?.color || '#818CF8' },
    { phase: 'Anestezie→Výkon', duration: 8, color: WORKFLOW_STEPS[1]?.color || '#A855F7' },
    { phase: 'Chirurgický výkon', duration: 95, color: WORKFLOW_STEPS[2]?.color || '#06B6D4' },
    { phase: 'Výkon→Konec anestezie', duration: 15, color: WORKFLOW_STEPS[3]?.color || '#10B981' },
    { phase: 'Anestezie→Úklid', duration: 10, color: WORKFLOW_STEPS[4]?.color || '#F97316' },
    { phase: 'Úklid→Připraveno', duration: 20, color: WORKFLOW_STEPS[5]?.color || '#FBBF24' },
  ];
          ...
          <div className="space-y-8 py-8 px-2">
            ...
            >
              {[
                ...
                { label: 'Vytížení', value: '87%', icon: TrendingUp, color: colors.success },
                { label: 'Cyklus (min)', value: '145', icon: Clock, color: colors.warning },
                { label: 'Efektivita', value: '89%', icon: Activity, color: colors.secondary },
              ].map((metric, idx) => {
                const Icon = metric.icon;
                return (
                  <motion.div
            ...
            >
              <h2 className="text-lg font-black text-white mb-6 flex items-center gap-2">
                <Clock className="w-5 h-5" style={{ color: colors.info }} />
                Rozpis jednotlivých fází operačního cyklu
              </h2>
              <div className="space-y-3">
                {phaseData.map((phase, idx) => {
                  const percentage = ((phase.duration / totalDuration) * 100).toFixed(1);
